...
def compute_loss(real, pred):  # def loss_function(real, pred)
...
if __name__ == '__main__':
    ...
    restore_epoch = int(opt.ckp_restore_path.split('epoch')[-1].split('_')[-1].split('/')[0])
    ...
    with strategy.scope():
        ...
        for epoch in range(opt.epochs):
            ...
            print(template.format(epoch + 1 + restore_epoch, train_loss, train_accuracy.result() * 100)); sys.stdout.flush();

            print('### Save the checkpoints ...')
            if (epoch + 1 + opt.restore_epoch) % 1 == 0:  # if (epoch + 1) % 5 == 0:
