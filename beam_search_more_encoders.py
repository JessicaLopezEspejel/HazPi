...
def initialization_vars(encoder_input, summary_tokenizer, transformer, opt):
    ...
    predictions, attention_weights = transformer(encoder_input, outputs, False, enc_padding_mask, combined_mask,
                                                 dec_padding_mask)
...
def summarize(input_document, summary_tokenizer, document_tokenizer, transformer, opt):
    ...
    if ind is None:
        ...
        try:
            ...
            print("length: ", length, 'type: ', type(length),  ' ind: ', ind, 'type: ', type(ind))
...
if __name__ == '__main__':
    ...
    for idx, (doc, gold_summary) in enumerate(zip(documents_pass, summary_gold)):
        try:
            encoded_path_txt = os.path.join(opt.path_summaries_encoded, str(ind_sum + idx).zfill(6) + '_encoder.txt')
            decoded_path_txt = os.path.join(opt.path_summaries_decoded, str(ind_sum + idx).zfill(6) + '_decoder.txt')
            if os.path.exists(encoded_path_txt) and os.path.exists(decoded_path_txt):
                print('summary {} exists = '.format(str(ind_sum + idx).zfill(6)))
                continue
